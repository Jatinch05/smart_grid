...
"Dfvp7OOGAN6dEOM4+qR9sdjoSYKEBpsr6GtPAQw4dy753ec5qwerty\n" \
"-----END CERTIFICATE-----\n";
...
void setup() {
  ...
  if (client.connect("ESP32SecureClient", mqtt_user, mqtt_password)) {
    ...
    Serial.println(" (Check MQTT broker settings, credentials, and network connection)");
  }
}

void loop() {
  // Maintain the MQTT connection and process any messages
  if (!client.connected()) {
